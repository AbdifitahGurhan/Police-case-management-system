...
async function run() {
  ...
  await db.query(`
    ...
    LEFT JOIN regions r ON r.id=COALESCE(po.region_id,c.region_id)
    LEFT JOIN users creator ON creator.username=po.created_by
    SET po.region_id=COALESCE(po.region_id,c.region_id,creator.region_id),
        ...
        po.registration_region_id=COALESCE(po.registration_region_id,po.region_id,c.region_id,creator.region_id),
  ...
  await db.query(`UPDATE police_officers po JOIN officer_assignments a ON a.officer_id=po.id AND a.is_current=1 JOIN cities c ON c.id=a.assignment_id JOIN regions r ON r.id=c.region_id SET po.state_administration_id=r.state_administration_id,po.region_id=r.id,po.district_id=NULL WHERE a.assignment_type='City'`);
  await db.query(`UPDATE police_officers po JOIN officer_assignments a ON a.officer_id=po.id AND a.is_current=1 JOIN districts d ON d.id=a.assignment_id JOIN cities c ON c.id=d.city_id JOIN regions r ON r.id=c.region_id SET po.state_administration_id=r.state_administration_id,po.region_id=r.id,po.district_id=d.id WHERE a.assignment_type IN ('District','District Station')`);
